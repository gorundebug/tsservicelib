...
const transportPaths = [
  "datasource/http/node-http.ts", "datasink/http/node-http.ts",
  "datasource/grpc/grpc-js.ts", "datasink/grpc/grpc-js.ts",
  "datasource/kafka/confluent.ts", "datasink/kafka/confluent.ts",
  "datasource/localsource/custom.ts", "datasink/localsink/custom.ts",
  "datasource/temporal/temporal.ts", "datasink/temporal/temporal.ts",
...
function hasSpanGuard(condition: ts.Expression, span: ts.Expression, file: ts.SourceFile): boolean {
  ...
  return condition.operatorToken.kind === ts.SyntaxKind.ExclamationEqualsEqualsToken
...
for (const path of transportPaths) {
  await test(`error tracing is guarded before evaluating arguments: ${path}`, async () => {
    ...
    function visit(node: ts.Node): void {
      if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === "spanError") {
        calls += 1;
        ...
        while (parent !== undefined) {
          if (ts.isIfStatement(parent) && parent.thenStatement === child
            && hasSpanGuard(parent.expression, span, file)) guarded = true;
          child = parent;
          parent = parent.parent;
        }
        const position = file.getLineAndCharacterOfPosition(node.getStart(file));
        assert.ok(guarded, `${path}:${String(position.line + 1)} invokes spanError without a caller-side guard`);
      }
...
await test("untraced durable errors do not read trace metadata but still report diagnostics", () => {
  ...
  Object.defineProperty(failure, "message", { get: () => { messageReads += 1; return "business failure"; } });
  ...
  const durable = new DurableCallContext("request", "Activity", {
    diagnostics: (event, error) => { events.push({ event, error }); }
  });
  ...
  assert.deepEqual(events.map(({ event }) => event), ["heartbeat", "error"]);
...
await test("traced durable errors retain lifecycle events, error status, and original attributes", () => {
  ...
  const started = tracing.tracer("service").start(new MessageContext().withSampling(true), "temporal.activity");
  const durable = new DurableCallContext("request", "Activity");
  ...
  assert.deepEqual(span.events.map(({ name }) => name), ["temporal.activity.heartbeat", "temporal.activity.error"]);
  assert.deepEqual(span.events[0]?.attributes, []);
  assert.deepEqual(span.events[1]?.attributes, [{ key: "error", type: "string", value: "business failure" }]);
});
