...
const asyncHooks =
  typeof process === "undefined" ? undefined : process.getBuiltinModule("async_hooks");
const os = typeof process === "undefined" ? undefined : process.getBuiltinModule("os");

export function bindPoolTask(execute: PoolTask): PoolTask {
  return asyncHooks?.AsyncResource.bind(execute) ?? execute;
}

export function normalizeExecutorsCount(count: number): number {
...
const subscriptions = new WeakMap<
  AbortSignal,
  { callbacks: Set<() => void>; listener: () => void }
>();
export function subscribeAbort(signal: AbortSignal, callback: () => void): () => void {
  ...
  return () => {
    ...
    if (current.callbacks.size === 0) {
      ...
      if (subscriptions.get(signal) === current) subscriptions.delete(signal);
    }
  };
}
