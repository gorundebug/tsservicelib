...
export class TemporalConnector implements ManagedDataConnector {
      ...
      try {
        ...
        this.#environment
          ...
          .warn(
            ...
            err(shutdownError instanceof Error ? shutdownError : new Error(String(shutdownError)))
          );
      }
      try {
        ...
        this.#environment
          ...
          .warn(
            context,
            "Temporal connection startup rollback failed",
            str("connector", this.name),
            err(closeError instanceof Error ? closeError : new Error(String(closeError)))
          );
      }
  ...
  public async stop(context: Context): Promise<void> {
    ...
    try {
      ...
      if (shutdownError === undefined) shutdownError = closeError;
      else {
        this.#environment
          .log()
          .warn(
            context,
            "Temporal connection shutdown failed after Worker failure",
            str("connector", this.name),
            err(closeError instanceof Error ? closeError : new Error(String(closeError)))
          );
      }
  ...
  private queuePolicy(taskQueue: string): {
    ...
    for (const registration of this.#endpoints.values()) {
      ...
      if (config.temporalExecutionType === "Activity") {
        if (
          ...
          throw new Error(
            `Temporal Task Queue ${taskQueue} has conflicting maxConcurrentActivities`
          );
        }
        maxConcurrentActivities = config.maxConcurrentActivities;
      } else {
        if (
          ...
          throw new Error(
            `Temporal Task Queue ${taskQueue} has conflicting maxConcurrentWorkflowTasks`
          );
        }
        maxConcurrentWorkflowTasks = config.maxConcurrentWorkflowTasks;
      }
...
function resolveWorkerStopTimeout(configuredMillis: number, serviceShutdownMillis: number): number {
